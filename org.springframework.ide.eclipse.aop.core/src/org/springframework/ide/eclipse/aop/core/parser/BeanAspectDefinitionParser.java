...
public class BeanAspectDefinitionParser {
    ...
    protected static class AspectJAnnotation<A extends Annotation> {
        ...
        static {
            ...
            annotationTypes.put(Pointcut.class,
        ...
        private static final String[] EXPRESSION_PROPERTIES = new String[] {
                "value", "pointcut" };
        ...
        public AspectJAnnotation(A aspectjAnnotation) {
            ...
            try {
                ...
                this.argNames = (String) annotation.getClass().getMethod(
                        "argNames", (Class[]) null).invoke(this.annotation);
            }
            catch (Exception ex) {
     ...
     */
    protected static AspectJAnnotation findAspectJAnnotationOnMethod(
            Method aMethod) {
    ...
    private static AspectJExpressionPointcut getPointcut(
            Method candidateAspectJAdviceMethod, Class<?> candidateAspectClass) {
    ...
    private static boolean isAspect(Class<?> clazz) {
        boolean couldBeAtAspectJAspect = AjTypeSystem.getAjType(clazz)
                .isAspect();
                ...
                if (className != null && isIncluded(patternList, id)) {
                    try {
                        ...
                        if (isAspect(aspectClass) && validate(aspectClass)) {
                            createAnnotationAspectDefinition(document, bean,
        ...
        ReflectionUtils.doWithMethods(aspectClass,
                new ReflectionUtils.MethodCallback() {
                    public void doWith(Method method)
                        ...
                        // Exclude pointcuts
                        if (AnnotationUtils.getAnnotation(method,
                                Pointcut.class) == null) {
                            AspectJExpressionPointcut ajexp = getPointcut(
                                    method, aspectClass);
                            if (ajexp == null) {
                                return;
                            }
                            ...
                            if (aspectJAnnotation.getArgNames() != null) {
                                def.setArgNames(StringUtils
                                        .commaDelimitedListToStringArray(aspectJAnnotation
                                                .getArgNames()));
                            }
                            switch (aspectJAnnotation.getAnnotationType()) {
                                ...
                                case AtAfterReturning:
                                    ...
                                    if (StringUtils.hasText(afterReturningAnnotation.returning())) {
                                        def.setReturning(afterReturningAnnotation
                                                .returning());
                                    }
                                    break;
                                case AtAfterThrowing:
                                    ...
                                    if (StringUtils.hasText(afterThrowingAnnotation.throwing())) {
                                        def.setThrowing(afterThrowingAnnotation
                                                .throwing());
                                    }
                                    break;
                                case AtAround:
        ...
        if (StringUtils.hasText(className)) {
            ...
            for (int g = 0; g < aspectChildren.getLength(); g++) {
                ...
                if ("before".equals(aspectNode.getLocalName())) {
                    BeanAspectDefinition info = parseAspect(pointcuts,
                            rootPointcuts, aspectNode,
    ...
    private static boolean validate(Class<?> aspectClass)
            throws AopConfigException {
        // If the parent has the annotation and isn't abstract it's an error
        if (aspectClass.getSuperclass().getAnnotation(Aspect.class) != null
                && !Modifier.isAbstract(aspectClass.getSuperclass()
                        .getModifiers())) {
            return false;
