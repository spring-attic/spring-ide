 ...
 * @since 3.2
 */
public class RooContentProposalAdapter {
	...
	class ContentProposalPopup extends PopupDialog {
		...
		private class InfoPopupDialog extends PopupDialog {
			...
			protected void adjustBounds() {
				System.out.println("InfoPopup adjustBounds!");
				...
				if (rightProposedBounds.intersects(parentBounds)) {
					...
					if (leftProposedBounds.intersects(parentBounds)) {
						...
						if (rightProposedBounds.x - parentBounds.x >= parentBounds.x
							...
							int newX = parentBounds.x
									+ parentBounds.width
									+ PopupDialog.POPUP_HORIZONTALSPACING;
							int shrinkBy = newX - rightProposedBounds.x;
							rightProposedBounds.x = newX;
							rightProposedBounds.width -= shrinkBy;
							//<<< BUGFIX STS
							proposedBounds = rightProposedBounds;
						} else {
							System.out.println("InfoPopup adjustBounds (4)");
						...
						System.out.println("InfoPopup adjustBounds (5)");
						// use the proposed bounds on the left
						proposedBounds = leftProposedBounds;
					}
				} else {
					// use the proposed bounds on the right
					System.out.println("InfoPopup adjustBounds (6)");
					proposedBounds = rightProposedBounds;
				}
...
//Spring Change: removed see proposalListeners2
//			notifyPopupClosed();
			return ret;
		}
...
//Spring Change: removed. Not compatible with this copy because IContentProposalListener2
// has references to original type which is too locked down to be subclassed properly.
...
//	private ListenerList proposalListeners2 = new ListenerList();
	 ...
	 */
	public RooContentProposalAdapter(Control control,
