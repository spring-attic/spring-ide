...
public class InstalledRooInstallBlock implements ISelectionProvider {
	 ...
	 * VMs being displayed
	 */
	private final List<IRooInstall> fVMs = new ArrayList<IRooInstall>();
	 ...
	 * @since 3.2
	 */
	public String generateName(String name, IRooInstall install) {
	 ...
	 */
	public IRooInstall getCheckedJRE() {
		Object[] objects = fVMList.getCheckedElements();
		if (objects.length == 0) {
			return null;
		}
		return (IRooInstall) objects[0];
	}
	 ...
	 */
	public IRooInstall[] getJREs() {
		return fVMs.toArray(new IRooInstall[fVMs.size()]);
	}
	...
	public boolean isDuplicateName(String name, IRooInstall install) {
		if (install != null) {
	 ...
	 * @param vms
	 */
	public void removeJREs(IRooInstall[] vms) {
	 ...
	 */
	public void setCheckedJRE(IRooInstall vm) {
		if (vm == null) {
	...
	public void vmAdded(IRooInstall vm) {
		fVMs.add(vm);
		fVMList.refresh();
	}
	...
	private void removeVMs() {
		IStructuredSelection selection = (IStructuredSelection) fVMList.getSelection();
		IRooInstall[] vms = new IRooInstall[selection.size()];
		Iterator<IRooInstall> iter = selection.iterator();
		int i = 0;
		while (iter.hasNext()) {
			vms[i] = iter.next();
			i++;
	...
	protected void fillWithWorkspaceJREs() {
		Collection<IRooInstall> installs = RooCoreActivator.getDefault().getInstallManager().getAllInstalls();
		setJREs(installs.toArray(new IRooInstall[installs.size()]));
	}
	 ...
	 */
	protected void setJREs(IRooInstall[] vms) {
		fVMs.clear();
		for (IRooInstall vm : vms) {
			fVMs.add(vm);
		}
