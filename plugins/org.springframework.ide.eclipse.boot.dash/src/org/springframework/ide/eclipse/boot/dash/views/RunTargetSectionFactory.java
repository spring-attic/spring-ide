...
class RunTargetSectionFactory implements SectionFactory<BootDashModel> {
	...
	public IPageSection create(BootDashModel model) {
		if (model instanceof TargetBootDashModel) {
			String sectionName = ((TargetBootDashModel) model).getRunTarget().getName();
			BootDashElementsTableSection section = new BootDashElementsTableSection(owner, model,
					new TagFilterBoxModel().getFilter());
			section.setColumns(RUN_STATE_ICN, PROJECT);
