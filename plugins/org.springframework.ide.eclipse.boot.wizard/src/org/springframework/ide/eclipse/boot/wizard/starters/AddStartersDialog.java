 ...
 */
public class AddStartersDialog extends DialogWithSections {
	...
	public InitializrFactoryModel<AddStartersModel> model;
	private boolean firstSearchBox = true; //becomes false after the searchBox is created first time.
	private long openingTime;

	public AddStartersDialog(InitializrFactoryModel<AddStartersModel> model, Shell shell) {
		super("Add Spring Boot Starters in Existing Project", model, shell);
	...
	protected WizardPageSection createDynamicContents(AddStartersModel model) {
		AddStartersDialog owner = AddStartersDialog.this;
...
//	}
	...
	private SearchBoxSection searchBox(AddStartersModel model) {
	...
	public static int openFor(IProject selectedProject, Shell shell) throws CoreException {
		...
		InitializrFactoryModel<AddStartersModel> fmodel = new InitializrFactoryModel<>((url) -> {
			if (url!=null) {
		...
		return new AddStartersDialog(fmodel, shell).open();
	}
