...
RED.nodes = function () {
    ...
    var colors = ["rgb(253,208,162)","rgb(135,169,128)","rgb(165,187,207)"];

    function getType(type) {
        if (node_defs[type] == null) {
            node_defs[type] = new Object();
            node_defs[type].category = "Blah";
            node_defs[type].outputs = 1;
            node_defs[type].inputs = 1;
            var ci = (Math.floor(Math.random() * 3));
            node_defs[type].color = colors[ci];
        }
