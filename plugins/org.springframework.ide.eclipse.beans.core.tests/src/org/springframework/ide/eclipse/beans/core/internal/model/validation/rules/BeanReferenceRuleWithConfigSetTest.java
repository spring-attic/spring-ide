 ...
 * @since 3.6.3
 */
public class BeanReferenceRuleWithConfigSetTest extends BeansCoreTestCase {
	...
	@Test
	public void testBeanReferenceNotFoundToBeanFromProfileComposite() throws Exception {
		...
		assertEquals(1, markers.length);
		String msg = (String) markers[0].getAttribute(IMarker.MESSAGE);
		assertEquals("Referenced bean 'profileEmbeddedFoo' not found [config set: validation-beanreference/testset]", msg);
	}
	...
	@Test
	public void testBeanReferenceNoMarkersInProfileSection() throws Exception {
		Set<IMarker> markers = MarkerUtils.getAllMarkersInRange(resource, 37, 57);
		assertEquals(0, markers.size());
	}
