...
public class IntFileModelFactory extends AbstractIntegrationModelFactory {
	...
	public void getChildrenFromXml(List<Activity> list, IDOMElement input, Activity parent) {
		if (input.getLocalName().equals(IntFileSchemaConstants.ELEM_FILE_TO_BYTES_TRANSFORMER)) {
			FileToBytesTransformerModelElement transformer = new FileToBytesTransformerModelElement(input,
					parent.getDiagram());
			list.add(transformer);
		}
		else if (input.getLocalName().equals(IntFileSchemaConstants.ELEM_FILE_TO_STRING_TRANSFORMER)) {
			FileToStringTransformerModelElement transformer = new FileToStringTransformerModelElement(input,
					parent.getDiagram());
			list.add(transformer);
		}
		...
		else if (input.getLocalName().equals(IntFileSchemaConstants.ELEM_TAIL_INBOUND_CHANNEL_ADAPTER)) {
			TailInboundChannelAdapterModelElement adapter = new TailInboundChannelAdapterModelElement(input,
					parent.getDiagram());
			list.add(adapter);
		}
	}
