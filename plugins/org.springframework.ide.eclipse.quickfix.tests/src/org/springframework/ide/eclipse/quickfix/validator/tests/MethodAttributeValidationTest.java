...
public class MethodAttributeValidationTest extends AbstractBeanValidationTestCase {
	...
	public void testDeprecatedDestroyMethod() {
		assertTrue(
				"Expects error in destroy-method",
				hasMethodError("deprecatedDestroyMethod", BeansSchemaConstants.ATTR_DESTROY_METHOD,
	...
	public void testDeprecatedInitMethod() {
		assertTrue("Expects error in init-method",
				hasMethodError("deprecatedInitMethod", BeansSchemaConstants.ATTR_INIT_METHOD, initDestroyAttrValidator));
		List<IMessage> messages = reporter.getMessages();
	...
	public void testDestroyMethodWithError() {
		assertTrue(
				"Expects error in destroy-method",
				hasMethodError("destroyMethodTest2", BeansSchemaConstants.ATTR_DESTROY_METHOD, initDestroyAttrValidator));
		List<IMessage> messages = reporter.getMessages();
	...
	public void testDestroyMethodWithNoError() {
		assertFalse(
				...
				hasMethodError("destroyMethodTest1", BeansSchemaConstants.ATTR_DESTROY_METHOD, initDestroyAttrValidator));
		List<IMessage> messages = reporter.getMessages();
	...
	public void testFactoryMethodWithError() {
		assertTrue(
				"Expects error in factory-method",
				hasMethodError("factoryMethodTest2", BeansSchemaConstants.ATTR_FACTORY_METHOD,
						factoryMethodAttrValidator));
		List<IMessage> messages = reporter.getMessages();
	...
	public void testFactoryMethodWithNoError() {
		assertFalse(
				"Does not expect error",
				hasMethodError("factoryMethodTest1", BeansSchemaConstants.ATTR_FACTORY_METHOD,
						factoryMethodAttrValidator));
	...
	public void testInitMethodWithError() {
		assertTrue("Expects error in init-method",
				hasMethodError("initMethodTest2", BeansSchemaConstants.ATTR_INIT_METHOD, initDestroyAttrValidator));
		List<IMessage> messages = reporter.getMessages();
	...
	public void testInitMethodWithNoError() {
		assertFalse("Does not expect error",
				hasMethodError("initMethodTest1", BeansSchemaConstants.ATTR_INIT_METHOD, initDestroyAttrValidator));
		List<IMessage> messages = reporter.getMessages();
		assertEquals("Expects no messages", 0, getVisibleMessages(messages).size());
		assertNotNull("Expects RenameMethodQuickAssistProcessor to be in reporter",
				getProcessor(messages, RenameMethodQuickAssistProcessor.class));
