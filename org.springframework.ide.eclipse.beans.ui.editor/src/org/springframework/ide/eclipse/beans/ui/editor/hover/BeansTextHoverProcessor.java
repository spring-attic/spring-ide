...
public class BeansTextHoverProcessor
    ...
    protected String computeHoverHelp(ITextViewer textViewer, int documentPosition) {
        ...
        if (flatNode != null) {
            ...
            if (region != null) {
                result = computeRegionHelp(treeNode, parentNode, flatNode, region, textViewer
                        .getDocument());
            }
        }
    ...
    protected String computeRegionHelp(IndexedRegion treeNode, IDOMNode parentNode,
            IStructuredDocumentRegion flatNode, ITextRegion region, IDocument document) {
        String result = null;
        if (region == null)
            return null;
    ...
    protected String computeTagAttValueHelp(IDOMNode xmlnode, IDOMNode parentNode,
        ...
        if ("class".equals(attName) && attributes.getNamedItem("class") != null) {
            ...
            for (int i = 0; i < classNames.size(); i++) {
                try {
                    IMethod method = Introspector.getWritableProperty((IType) classNames.get(i),
                            propertyName);
                    if (method != null) {
