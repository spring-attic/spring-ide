...
public class UtilContentAssistProcessor
    ...
    public UtilContentAssistProcessor() {
    ...
    private void addCollectionTypesAttributeValueProposals(
        ...
        IType type = BeansModelUtils.getJavaType(getResource(request)
                .getProject(), typeName);
        try {
            if (getResource(request).getProject().hasNature(JavaCore.NATURE_ID)) {

                ITypeHierarchy hierachy = type.newTypeHierarchy(JavaCore
                        .create(getResource(request).getProject()),
                    ...
                    if (!sortMap.containsKey(foundType.getFullyQualifiedName())
                        ...
                        BeansJavaCompletionProposal proposal = new BeansJavaCompletionProposal(
                                foundType.getFullyQualifiedName(), request
                                        .getReplacementBeginPosition(), request
                                        .getReplacementLength(), foundType
                                        .getFullyQualifiedName().length(),
                                JavaPluginImages
            ...
            delegatingContextAssistProcessor
                    .setErrorMessage("Error ocurred during content assist");
        }
