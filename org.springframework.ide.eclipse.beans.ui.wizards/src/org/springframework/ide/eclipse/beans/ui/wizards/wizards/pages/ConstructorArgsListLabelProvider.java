...
public class ConstructorArgsListLabelProvider extends LabelProvider {
	...
	private Image fConstructorImage;

	private Image fConstructorArgImage;

	public ConstructorArgsListLabelProvider() {
		super();
		fPropertyImage = BeansUIImages
				.getImage(BeansUIImages.IMG_OBJS_PROPERTY);
		...
		fMapEntryImage = BeansUIImages
				.getImage(BeansUIImages.IMG_OBJS_DESCRIPTION);
		fPropImage = BeansUIImages.getImage(BeansUIImages.IMG_OBJS_DESCRIPTION);
		fConstructorImage = BeansUIImages
				.getImage(BeansUIImages.IMG_OBJS_CONSTRUCTOR);
		// FIXME find a proper image for a constructor argument
		fConstructorArgImage = BeansUIImages
				.getImage(BeansUIImages.IMG_OBJS_DESCRIPTION);
	}
	...
	public Image getImage(Object element) {
		...
		if (element instanceof ConstructorModelItem) {
			result = fConstructorImage;
		}
		if (element instanceof ConstructorArgModelItem) {
			result = fConstructorArgImage;
		}
		return result;
	}
	...
	public String getText(Object element) {
			...
			if (propertyModelItem.getType() != null) {
				...
				if (propertyModelItem.isPrimitive()) {
					result = propertyModelItem.getName() + "<"
							+ propertyModelItem.getPrimitiveTypeName() + ">";
				} else {
					result = propertyModelItem.getName() + "<???>";
				}
			}
		...
		if (element instanceof ConstructorModelItem) {
			ConstructorModelItem constructorModelItem = ((ConstructorModelItem) element);
			result = SpringBeanConstructorWizardPage
					.createConstructorId(constructorModelItem);
		}
		if (element instanceof ConstructorArgModelItem) {
			ConstructorArgModelItem constructorArgModelItem = ((ConstructorArgModelItem) element);
			if (constructorArgModelItem.isPrimitive()) {
				result = constructorArgModelItem.getName() + "<"
						+ constructorArgModelItem.getPrimitiveTypeName() + ">";
			} else {
				result = constructorArgModelItem.getName() + "<"
						+ constructorArgModelItem.getType().getElementName()
						+ ">";
			}
		}
		return result;
