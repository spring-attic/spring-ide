...
public class BeansEditorFileDropAction
    ...
    public void dropResource(IResource resource[], IEditorPart editorPart) {
        try {
            if (resource != null && resource.length == 1) {
                ...
                if (typeDropped != null) {
                    IFile file = (IFile) editorPart.getEditorInput().getAdapter(IResource.class);
                    IJavaProject project = JavaCore.create(file.getProject());
                    if (project != null) {
                        IType type = project.findType(typeDropped.getFullyQualifiedName());
                        if (type == null) {
                            return;
                        }
                        try {
                            SpringBeanWizard springBeansDeclarationWizard = new SpringBeanWizard();
                            springBeansDeclarationWizard.setOriginatingType(type);
                            springBeansDeclarationWizard.init(PluginUtils.getWorkbench(),
                                    new StructuredSelection(new Object[] { file }));
                            new WizardFormsDialog(editorPart.getEditorSite().getShell(),
                                    springBeansDeclarationWizard).open();
                        }
                        catch (Throwable e) {
                            ErrorDialog errorDialog = new ErrorDialog("Spring IDE Error",
    ...
    private static IType getTypeInFile(IResource resource) {
        if (resource instanceof IFile) {
            ...
            else if (file.getProjectRelativePath().getFileExtension().equals("class")) {
