...
public class NewWebflowConfigFilePage extends WizardNewFileCreationPage {
	...
	protected InputStream createXMLDocument() throws Exception {
		...
		StringBuilder builder = new StringBuilder().append(
				"<flow xmlns=\"http://www.springframework.org/schema/webflow\"").append(
				lineSeparator).append("	xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"")
		...
		if (!swf2Button.getSelection()) {
			builder.append("	<start-state idref=\"start\" />").append(lineSeparator).append(
					lineSeparator);
		}
		builder.append("	<view-state id=\"start\">").append(lineSeparator).append("	</view-state>")
				.append(lineSeparator).append(lineSeparator).append("</flow>");
