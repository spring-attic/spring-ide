...
OSGI_VERSION=1.0-rc1
BUNDLE_OSGI_VERSION=1.0.0.rc1
...
download_source() {	
	...
	SRC_BUNDLE=$6
	REPO=$7
	FOLDER_NAME=./target/plugins/${SRC_BUNDLE}_${BUNDLE_VERSION}/src/${SYMBOLIC_NAME}_${BUNDLE_VERSION}
...
download_spring_source() {
	download_source org/springframework $1 $SPRING_VERSION $2 $BUNDLE_SPRING_VERSION org.springframework.spring.bundle.source http://repo1.maven.org/maven2
}

download_osgi_source() {
	download_source org/springframework/osgi $1 $OSGI_VERSION $2 $BUNDLE_OSGI_VERSION org.springframework.osgi.bundle.source http://s3.amazonaws.com/maven.springframework.org/milestone
}
