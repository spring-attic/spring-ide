...
public class Introspector {
	...
	public static final Collection findAllMethods(IType type,
			String methodPrefix, int argCount, boolean isPublic, int staticFlag)
			...
			for (int i = 0; i < methods.length; i++) {
				...
				if (!allMethods.containsKey(key)
						...
						&& (staticFlag == Introspector.STATIC_IRRELVANT
								|| (staticFlag == Introspector.STATIC_YES && Flags
										.isStatic(flags)) || (staticFlag == Introspector.STATIC_NO && !Flags
								.isStatic(flags)))
	 ...
	 * with specified features.
	 */
	public static final Collection findAllConstructors(IType type)
			throws JavaModelException {
