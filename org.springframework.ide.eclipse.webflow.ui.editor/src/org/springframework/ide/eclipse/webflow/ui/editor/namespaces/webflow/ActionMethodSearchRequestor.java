...
public class ActionMethodSearchRequestor extends PublicMethodSearchRequestor {
	...
	public void acceptSearchMatch(IMethod method) throws CoreException {
		if (Flags.isPublic(method.getFlags())
			...
			if (returnType != null && parameterTypes.size() == 1
				...
				/*
				 * else { IType eventType = BeansModelUtils.getJavaType(type
				 * .getJavaProject().getProject(), EVENT_CLASS); IType
				 * requestContextType = BeansModelUtils.getJavaType(type
				 * .getJavaProject().getProject(), REQUEST_CONTEXT_CLASS);
				 * ITypeHierarchy returnTypeHierachy = eventType
				 * .newSupertypeHierarchy(new NullProgressMonitor());
				 * ITypeHierarchy requestContextTypeHierachy =
				 * requestContextType .newSupertypeHierarchy(new
				 * NullProgressMonitor());
				 * 
				 * if (returnTypeHierachy.contains(returnType) &&
				 * requestContextTypeHierachy .contains(parameterTypes.get(0))) {
				 * createMethodProposal(method); } }
				 */
			}
		}
