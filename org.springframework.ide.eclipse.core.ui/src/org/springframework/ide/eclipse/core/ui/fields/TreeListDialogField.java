...
public class TreeListDialogField extends DialogField {
	 ...
	 */
	public TreeListDialogField(FormToolkit formToolkit, ITreeListAdapter adapter, String[] buttonLabels, ILabelProvider lprovider) {
		super(formToolkit);
		...
		treeStyle = SWT.BORDER | SWT.MULTI | SWT.H_SCROLL | SWT.V_SCROLL;
	}
	...
	public Control[] doFillIntoTable(Composite parent, int nColumns) {
		Control[] result = null;
		boolean anyButtons = true;
		if (fButtonLabels == null || fButtonLabels.length == 0) {
			anyButtons = false;
		}

		Label label = getLabelControl(parent);
		TableWrapData td = tableWrapDataForLabel(1);
		td.colspan = nColumns;
		td.rowspan = 1;
		...
		td.colspan = nColumns - (anyButtons ? 1 : 0);
		td.heightHint = new PixelConverter(parent).convertHeightInCharsToPixels(20);
		...
		if (anyButtons) {
			...
			result = new Control[] { label, list, buttons };
		} else {
			result = new Control[] { label, list };
		}
		return result;
	}
	...
	protected TreeViewer createTreeViewer(Composite parent) {
		Tree tree = formToolkit.createTree(parent, getTreeStyle());
		tree.setFont(parent.getFont());
		return new TreeViewer(tree);
	}

	protected Button createButton(Composite parent, String label, SelectionListener listener) {
		Button button = formToolkit.createButton(parent, "", SWT.FLAT);
	...
	public Composite getButtonBox(Composite parent) {
		if (fButtonsControl == null) {
			...
			Composite contents = formToolkit.createComposite(parent, SWT.NONE);
	...
	public void setTreeStyle(int treeStyle) {
